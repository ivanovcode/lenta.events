        ...
        if (statusSetButtons[statusId].includes(target.data('id'))) {
            ...
            target.on("click", function () {
                ...
                if (post.status_id == 'approved') {
                    ...
                    showProgressBtn(e.target);
                    showPreload();
                    send('api/post', 'GET', values, function (data) {
                    }, function (data) {
                        renderFormPostEdit(data.data.post[0]);
                        showEditForm();
                        initFormPostEdit(e.parent, post.status_id, statusId, data.data.post[0]);
                        $(selectors.body).animate({ scrollTop: 0 }, "slow");
                        $(selectors.images).removeAttr("style");
                    }, e);
                    hideProgressBtn(e.target);
                    hidePreload();
